...
const CandidateJV = () => {
  ...
  const [entries, setEntries] = useState([
    { code: "", name: "", ct: "", description: "", debit: "", credit: "" },
  ]);
  ...
  const handleSave = async () => {
    if (!voucherNumber || !date || !job || !candidate) {
    ...
    try {
      ...
      const transactionPromises = entries
        .map((entry) => {
          ...
          const selectedCandidate = candidates.find(
            (cand) => cand.name === candidate
          );

          // Create debit transaction if debit amount exists
          if (parseFloat(entry.debit) > 0) {
            return axios.post(
              "/api/accounting/transactions/",
              {
                transactionType: "Candidate JV",
                date,
                reference: voucherNumber,
                description: entry.description,
                amount: parseFloat(entry.debit),
                account: selectedAccount._id,
                contraAccount: selectedAccount._id, // For JV, contra account might be different
          ...
          // Create credit transaction if credit amount exists
          if (parseFloat(entry.credit) > 0) {
            return axios.post(
              "/api/accounting/transactions/",
              {
                ...
                reference: voucherNumber,
                description: entry.description,
                amount: -parseFloat(entry.credit), // Negative for credit
                account: selectedAccount._id,
                contraAccount: selectedAccount._id,
                candidate: selectedCandidate?._id,
                job: job,
              },
              {
      ...
      toast.success("Candidate JV saved successfully");
      handleNew();
    } catch (err) {
      console.error("Save error", err);
      toast.error("Failed to save candidate JV");
    }
  };
                ...
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    <input
                      ...
                      value={entry.ct}
                      onChange={(e) =>
                        handleEntryChange(index, "ct", e.target.value)
                      }
                      className="w-full px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                      placeholder="CT"
                    />
