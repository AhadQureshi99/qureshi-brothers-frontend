...
const CashReceiptVoucher = () => {
  ...
  const calculateTotal = () => {
    const sum = entries.reduce(
      (acc, entry) => acc + (parseFloat(entry.receipt) || 0),
      0
    );
    setTotal(sum);
  };
  ...
  const handleSave = async () => {
    ...
    if (entries.some((entry) => !entry.code || !entry.receipt)) {
    ...
    try {
      ...
      const transactionPromises = entries.map((entry) => {
        ...
        return axios.post(
          "/api/accounting/transactions/",
          {
            ...
            amount: parseFloat(entry.receipt),
            account: cashAccount, // Cash account
            contraAccount: selectedAccount._id, // The account receiving the cash
          },
          {
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      toast.success("Cash receipt voucher saved successfully");
      handleNew();
    } catch (err) {
      console.error("Save error", err);
      toast.error("Failed to save cash receipt voucher");
    }
  };
  ...
  const handleNew = () => {
    ...
    setCashAccount("");
    setEntries([{ code: "", name: "", description: "", receipt: "" }]);
    setTotal(0);
  };
                ...
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    <input
                      ...
                      value={entry.receipt}
                      onChange={(e) =>
                        handleEntryChange(index, "receipt", e.target.value)
                      }
                      className="w-full px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                      placeholder="0.00"
                    />
