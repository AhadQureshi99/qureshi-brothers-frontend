...
const JobCategories = () => {
  ...
  const loadCategories = async () => {
    try {
      ...
      const response = await axios.get("/api/config/job-categories/", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCategory) {
        await axios.put(
          `/api/config/job-categories/${editingCategory._id}`,
          formData,
          {
            headers: {
        ...
        await axios.post("/api/config/job-categories/", formData, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
      ...
      toast.error(
        error.response?.data?.message || "Failed to save job category"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`/api/config/job-categories/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `/api/config/job-categories/${id}/toggle-status`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
